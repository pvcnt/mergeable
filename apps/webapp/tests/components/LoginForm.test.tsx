...
describe("login form", () => {
  ...
  test("submit button is disabled before base URL option and token are filled", async () => {
    // GIVEN an empty dialog with a pre-defined GitHub URL.
    render(
      <LoginForm onSubmit={() => Promise.resolve()} githubUrl="https://github.corp.com"/>
    );
      
    // THEN the URL input must be disabled.
    const urlInput = screen.getByRole("textbox", { name: "Base URL" });
    expect(urlInput.getAttribute("disabled")).toBe("");

...
export class SearchQuery {
    ...
    setAll(qualifier: string, values: string[]): undefined {
        this.delete(qualifier);
        values.forEach(value => this.terms.push(new SearchTerm({qualifier, value})));
    }
    ...
    private parseTerm(str: string, not: boolean): SearchTerm {
        ...
        if (colonPos > -1) {
            ...
            if (rangePos > -1) {
                ...
                return new SearchTerm({qualifier, op: SearchOp.GreaterThan, value, exclude});
            } else if (rhs.startsWith(">")) {
                const value = rhs.slice(1);
                return new SearchTerm({qualifier, op: SearchOp.AtLeast, value, exclude});
            } else if (rhs.startsWith("<=")) {
                const value = rhs.slice(2);
                return new SearchTerm({qualifier, op: SearchOp.LessThan, value, exclude});
            } else if (rhs.startsWith("<")) {
                const value = rhs.slice(1);
                return new SearchTerm({qualifier, op: SearchOp.AtMost, value, exclude});
            } else {
